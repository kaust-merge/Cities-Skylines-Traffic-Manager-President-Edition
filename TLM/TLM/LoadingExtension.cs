...
namespace TrafficManager
{
    public class LoadingExtension : LoadingExtensionBase
    {
        ...
        private UIButton _createButton(UIComponent c, string text, int y, MouseEventHandler eventClick)
        {
            var button = c.AddUIComponent<UIButton>();
        ...
        private UITextField CreateTextField(UIComponent c, string str, int y)
        {
            UITextField textfield = c.AddUIComponent<UITextField>();
        ...
        public override void OnLevelLoaded(LoadMode mode)
        {
            ...
            var y = 164f + 130f + 5f;
            checkBox.relativePosition = new Vector3(14f, y);
            y += 40;

            _goToField = CreateTextField(panel.component, "", (int)y);

            y += 40;
            var resetProductionBtn = _createButton(panel.component, "reset production", (int)y, clickResetProduction);
        ...
        private void clickResetProduction(UIComponent component, UIMouseEventParameter eventParam)
        {
            ushort buildingId = WorldInfoPanel.GetCurrentInstanceID().Building;
            // display the right checkbox state  
            var building = Singleton<BuildingManager>.instance.m_buildings.m_buffer[buildingId];
            ushort rateVal = Convert.ToUInt16(_goToField.text);

            BuildingInfo info = Singleton<BuildingManager>.instance.m_buildings.m_buffer[buildingId].Info;
            info.m_buildingAI.SetProductionRate(buildingId, ref Singleton<BuildingManager>.instance.m_buildings.m_buffer[buildingId], (byte)rateVal);

            Debug.Log("reseting to "+ rateVal + " production rate");
